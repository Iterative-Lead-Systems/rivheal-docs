# User Flows

Sequence diagrams for the key user journeys in RivHeal.

| Flow | File |
|---|---|
| Appointment Booking | [appointment-booking.md](./appointment-booking.md) |
| Real-time Queue Tracking | [queue-tracking.md](./queue-tracking.md) |
| Symptom Checker + LLM Triage | [symptom-checker-llm.md](./symptom-checker-llm.md) |
| Medication Adherence | [medication-adherence.md](./medication-adherence.md) |
| Telemedicine | [telemedicine.md](./telemedicine.md) |
