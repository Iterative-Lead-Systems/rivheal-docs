...
| `KEYCLOAK_DB_USER`, `KEYCLOAK_DB_PASSWORD` | Keycloak internal DB |
| `KEYCLOAK_CLIENT_SECRET` | API server client secret |
| `REDIS_PASSWORD` | Redis password |
| `JWT_SECRET`, `JWT_REFRESH_SECRET` | Token secrets |
| `ENCRYPTION_KEY` | PII encryption key |
| `IMAGE_TAG` | Docker image tag to deploy |
| `DOCKER_REGISTRY` | Container registry prefix |
| `ENABLE_AI_FEATURES` | Global AI toggle (propagated to API and ML service) |
